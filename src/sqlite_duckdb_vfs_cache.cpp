...
void DuckDBCachedFile::InitializeFromRemote() {
	...
	if (size >= 20) {
		data_t header[20];
		data_ptr_t header_ptr = nullptr; 
		auto header_handle = new_handle->Read(header_ptr, 20, 0);
		if (!header_handle.IsValid()) {
			throw IOException("Unable to read header of a remote SQLite database \"%s\"", path);
		}
		memcpy(header, header_ptr, 20);
		...
		if (is_sqlite && wal_mode) {
			...
			try {
				...
				if (journal_handle->GetFileSize() > 0) {
					data_ptr_t first_ptr = nullptr;
					auto first_handle = journal_handle->Read(first_ptr, 1, 0);
					if (!first_handle.IsValid()) {
						throw IOException("Unable to read first byte of a journal file of a remote SQLite database \"%s\"", path);
					}
					hot_journal = *first_ptr != 0;
				}
			} catch (const std::exception &e) {
				if (!SidecarConfirmedAbsent(e)) {
					throw IOException(
...
int DuckDBCachedFile::Read(void *buffer, int amount, sqlite3_int64 offset) {
	...
	auto buffer_handle = caching_handle->Read(buffer_ptr, static_cast<idx_t>(bytes_to_read), offset);
	if (!buffer_handle.IsValid()) {
		throw IOException("Unable to read %d bytes from a remote SQLite database \"%s\"", bytes_to_read, path);
	}
	memcpy(buffer, buffer_ptr, static_cast<size_t>(bytes_to_read));
